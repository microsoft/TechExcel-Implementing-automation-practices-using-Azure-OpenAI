...
2. The file named `site.py` contains the skeleton of a Streamlit dashboard. In the "Chat with Data" section, complete the functions named `handle_chat_prompt()` and `create_chat_completion()`.
